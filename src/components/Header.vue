<script setup>
import ContactWrapper from "@/components/Contact.vue";
import { Button } from "@/components/ui/button";
import Link from "./Link.vue";
import ClImage from "@/components/ClImage.vue";

import { Instagram } from "lucide-vue-next";
import { instagram } from "@/data/data.json"

const placeholder = "data:image/png;base64,";


</script>


<template>
  <header class="relative flex flex-col lg:flex-col">
    <div class="lg:hidden flex justify-center lg:flex">
      <img src="/logo.png" alt="logo" class="h-36 object-contain" />
    </div>
    <div class="relative">
      <img class="absolute top-0 left-0 w-full h-full aspect-[1.8] object-cover" :src="placeholder" alt="">
      <div class="relative max-w-[170vh] m-auto border-x-2 border-gray-800">
        <ClImage class="w-full object-cover aspect-[1.6] md:aspect-[1.8] xl:aspect-[2]"
          heroStyles="xl:object-[center_-80px]" id="Alou/xrvb2c83zcpfxhm6nwlj" pid="hero" />
        <nav class="lg:block hidden absolute top-0 items-center p-8 w-full">
          <div class="max-w-xl flex justify-center m-auto gap-16 xl:gap-24 2xl:gap-32">
            <Link class="text-[length:inherit]" variant="link" href="#quartet">Quartet</Link>
            <Link class="text-[length:inherit]" variant="link" href="#termine">Termine</Link>
            <Link class="text-[length:inherit]" variant="link" href="#media">Media</Link>
          </div>
        </nav>
        <img src="/logo_full.svg" alt="logo" class="hidden lg:block absolute top-0 left-0 p-4 w-[15%] object-contain" />
        <div class="absolute bottom-0 left-1/2 -translate-x-1/2 flex justify-between w-full items-end max-w-screen-3xl">


          <!-- <img :src="url" class="w-full object-cover aspect-[1.6] xl:aspect-[1.8]" id="Alou/xrvb2c83zcpfxhm6nwlj" /> -->

          <div class="opacity-0 lg:opacity-0">
            <img src="/logo_full.svg" alt="logo" class="invert h-[20vw] object-contain" />
          </div>

          <div class="m-4 xl:mx-[10vw] xl:my-[5vw] gap-4 flex">
            <ContactWrapper client:only>
              <!-- Standard Link Styling -->
              <Button class="text-white underline hover:text-accent" size="header" variant="outline">
                Kontakt
              </Button>
            </ContactWrapper>
            <Button class="text-white underline hover:text-accent" size="icon_header" variant="outline">
              <a :href="instagram">
                <Instagram class="w-4" />
              </a>
            </Button>
          </div>
        </div>
      </div>
    </div>



    <nav class="lg:hidden bg-background1 bg-cover items-center p-2 w-full">
      <div class="max-w-xl flex justify-around m-auto gap-8">
        <Link class="" variant="link" href="#quartet">Quartet</Link>
        <Link class="" variant="link" href="#termine">Termine</Link>
        <Link class="" variant="link" href="#media">Media</Link>
      </div>
    </nav>
  </header>
</template>



